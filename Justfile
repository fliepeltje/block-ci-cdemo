...
test:
    #!/bin/bash
    test_stat=`curl {{ commit_status_url }} | jq .tests_ok`
    ...
    curl -X POST {{ commit_tests_url }}

build:
    #!/bin/bash
    build_stat=`curl {{ commit_status_url }} | jq .build_ok`
    ...
    curl -X POST {{ commit_build_url }}

deploy:
    #!/bin/bash
    deploy_stat=`curl {{ commit_status_url }} | jq .deploy_ok`
